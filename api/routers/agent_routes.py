...
async def execute_custom_prompt_workflow(request: CustomPromptRequest, current_user: TokenData = Depends(get_current_user)):
    ...
    is_fin_keyword = (
        any(w in lower_prompt for w in ["pemasukan", "pendapatan", "revenue", "invoice", "tagihan", "operator", "telkomsel", "indosat", "xl", "smartfren", "pengeluaran", "beban", "opex", "listrik", "pln", "sewa lahan", "lahan", "genset", "biaya", "arus kas", "cash flow", "cashflow"])
    ...
    is_inv_keyword = any(w in lower_prompt for w in ["stok", "material", "baterai", "kabel", "closure", "odc", "kritis", "persediaan", "gudang", "beli", "pesan", "restock", "supplier", "purchase order", "po", "terima", "sampai", "tiba", "penerimaan", "pengiriman", "delivered", "transit", "approved", "setujui"])
    ...
    if u_role != "ADMIN" and u_tenant != "ALL":
        if u_tenant == "INVENTORY" and (is_hr_keyword or is_fin_keyword) and not is_inv_keyword:
            return {
                "parsed_intent": {"workflow_id": "tenant_boundary_restricted"},
                "action_type": "out_of_scope",
                "message": f"Akses Ditolak: Permintaan ini di luar ranah kewenangan Anda. Akun Anda ({current_user.username}) terdaftar khusus untuk Divisi Inventory & Logistik. Anda tidak memiliki akses ke data HR atau Keuangan perusahaan.",
                "generated_prs": [],
                "affected_items": []
            }
        if u_tenant == "HR" and (is_inv_keyword or is_fin_keyword) and not is_hr_keyword:
            return {
                "parsed_intent": {"workflow_id": "tenant_boundary_restricted"},
                "action_type": "out_of_scope",
                "message": f"Akses Ditolak: Permintaan ini di luar ranah kewenangan Anda. Akun Anda ({current_user.username}) terdaftar khusus untuk Divisi HR & Field Workforce. Anda tidak memiliki akses ke data Material Gudang atau Keuangan perusahaan.",
                "generated_prs": [],
                "affected_items": []
            }
        if u_tenant == "FINANCE" and (is_inv_keyword or is_hr_keyword) and not is_fin_keyword:
            return {
                "parsed_intent": {"workflow_id": "tenant_boundary_restricted"},
                "action_type": "out_of_scope",
                "message": f"Akses Ditolak: Permintaan ini di luar ranah kewenangan Anda. Akun Anda ({current_user.username}) terdaftar khusus untuk Divisi Keuangan & Akuntansi. Anda tidak memiliki akses ke data Material Gudang atau Data Personalia.",
                "generated_prs": [],
                "affected_items": []
            }
    ...
    try:
        ...
        if not workflow_id or route_result.get("is_unrelated"):
            return {
                ...
                "action_type": "out_of_scope",
                "message": "Maaf, permintaan ini di luar ranah operasional yang dapat saya proses. Saya adalah AI Copilot khusus manajemen inventaris, pemantauan stok material menara, dan penerbitan dokumen pengadaan PT Bali Towerindo Sentra Tbk.",
                "available_workflows": [],
                "email_sent": False,
                "generated_prs": [],
                "affected_items": [],
