"""
Script to generate official Dummy Faktur Penerimaan Barang PDF for Catalog & OCR testing.
"""

from pathlib import Path
import typst

downloads_dir = Path("web/static/downloads")
downloads_dir.mkdir(parents=True, exist_ok=True)
storage_dir = Path("storage/downloads")
storage_dir.mkdir(parents=True, exist_ok=True)

typ_content = r"""#set page(
    paper: "a4",
    margin: (x: 2cm, y: 2cm),
    header: [
        #grid(
...
typ_file = downloads_dir / "faktur_penerimaan_barang_dummy.typ"
pdf_file = downloads_dir / "faktur_penerimaan_barang_dummy.pdf"
pdf_storage = storage_dir / "faktur_penerimaan_barang_dummy.pdf"

with open(typ_file, "w", encoding="utf-8") as f:
    f.write(typ_content)

typst.compile(str(typ_file), output=str(pdf_file))
typst.compile(str(typ_file), output=str(pdf_storage))

print(f"Generated dummy PDF successfully at: {pdf_file.resolve()}")
